...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $product->status         = 0;
        $product->shipping_cost  = $request->product_type == 'physical' ? $request->shipping_cost : 0;
        ...
        if ($request->ajax()) {
            ...
            $product->meta_title = $request->filled('meta_title') ? $request->meta_title : null;
            $product->meta_description = $request->filled('meta_description') ? $request->meta_description : null;
            $product->meta_image = $request->filled('meta_image') ? ImageManager::upload('product/meta/', 'png', $request->meta_image) : null;
            $product->save();
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->discount_type     = $request->discount_type;
        $product->current_stock     = $request->product_type == 'physical' ? abs($stock_count) : 0;
        $product->shipping_cost     = $request->product_type == 'physical' ? (Helpers::get_business_settings('product_wise_shipping_cost_approval')==1?$product->shipping_cost:($request->shipping_cost)) : 0;
        $product->multiply_qty      = ($request->product_type == 'physical') ? ($request->multiplyQTY=='on'?1:0) : 0;

        if(Helpers::get_business_settings('product_wise_shipping_cost_approval')==1 && $product->shipping_cost != ($request->shipping_cost))
        {
            $product->temp_shipping_cost = ($request->shipping_cost);
